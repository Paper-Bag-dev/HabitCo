    ...
    <div className="flex min-h-screen text-white bg-black/95 justify-center items-center font-montserrat">
      <div className="bg-[#222222] flex flex-col w-[400px] h-[600px] rounded-xl p-4 text-center items-center font-montserrat">
        ...
        <form onSubmit={submitHandler} className=" h-full w-full items-center flex flex-col justify-center">
          <span className=" w-full p-2 text-3xl text-[#bbbbbb]">Login</span>

          <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="Enter Email" className="bg-[#313131] border-0 mt-10 mb-5 text-[#c4c4c4] rounded-xl h-12 w-72" required/>
          <input value={password} onChange={(e) => setPassword(e.target.value)} type={visible ? "text" : "password"} placeholder="Enter Password" className={`bg-[#313131] text-lg border-0 my-5 text-[#c4c4c4] rounded-xl h-12 w-72 z-0`} required/>
          
          <div className="fixed ml-60 mt-3"> 
          ...
          <button disabled={loading} className="bg-[#313131] border-0 mt-5 text-[#c4c4c4] rounded-xl hover:bg-green-600 hover:text-white transition-all h-12 w-72" >Login</button>
            <span className="my-4 text-[#868686]">Or</span>
            <span className=" hover:cursor-pointer underline text-[#d6d6d6]"><Link to="/register">Register</Link></span>
        </form>
